#1P_XdateR - Beta 2 - **paper version 17/07/2025**
...
runtitle <- "Test"
#=================================
#Tucson file input and detrending - both data-sets need to be of the same parameter (RW, LWBI etc)
referenceTR <- read.rwl ("LGLlwbiTRUNC", header = FALSE) #read in reference file data
undatedTR <- read.rwl ("LCLlwbiTRUNC", header = FALSE) #read in undated file data
...
Tthreshold <- 3.5 #threshold of initial T-value acceptance
...
log_pval <- pt(Tvalue, df = NadjfromT$Nadj, lower.tail = FALSE, log.p = TRUE)
rawPvalue <- exp(log_pval) #1 tailed p-value
BonADJPvalue <- rawPvalue*NoCorrelations # this is the Bonferroni adjustment of the raw P value: P x number of correlations
minadjP <- min(BonADJPvalue)
nextminadjP <- min(BonADJPvalue[BonADJPvalue != minadjP]) #2nd highest 1/p
IF <- round((nextminadjP/minadjP), 0) #isolation factor
#however, many adjP values might be > 1 - these need to be adjusted to they are never > 1
minadjP <- min(BonADJPvalue)
BonADJPvalue <- ifelse(BonADJPvalue >1 ,1, BonADJPvalue) #anything large than 1 gets reset to 1
...
#correlation value for comparison plot
...
upper <- ggarrange(Ttimeseries, Ptimeseries, ncol = 1, nrow = 2, align = "v")
finalfigure <- ggarrange(upper, lower, ncol = 1, nrow = 2, heights = c(1.75, 1), align = "v")
